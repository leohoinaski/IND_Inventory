...
#%% Creating a dataset
def createNETCDF(folder,name,data,xv,yv,lat,lon,center,year,month):
    cdate = datetime.datetime.now()
    cdateStr = int(str(cdate.year)+str(cdate.timetuple().tm_yday))
    ctime = int(str(cdate.hour)+str(cdate.minute)+str(cdate.second))
    if month<10:
        sdate = int(str(year)+'00'+str(month)) 
    else:
        sdate = int(str(year)+'0'+str(month)) 
    ...
    f2.GDNAM= ''       
    f2.UPNAM= ''   
    strVAR = 'PM         CO              NOx             SOx         VOC'       
    f2.VAR_LIST=strVAR
    f2.FILEDESC= 'Data from industrial emissions inventory without chemical speciation'
    f2.HISTORY ='' 
    ...
    PM = f2.createVariable('PM', 'f4', ('TSTEP', 'LAY', 'ROW','COL'))
    CO = f2.createVariable('CO', 'f4', ('TSTEP', 'LAY', 'ROW','COL'))
    NOx = f2.createVariable('NOx', 'f4', ('TSTEP', 'LAY', 'ROW','COL'))
    SOx = f2.createVariable('SOx', 'f4', ('TSTEP', 'LAY', 'ROW','COL'))
    VOC = f2.createVariable('VOC', 'f4', ('TSTEP', 'LAY', 'ROW','COL'))
...
def createNETCDFtemporal(folder,name,data,xv,yv,lat,lon,dates,month,outType):
    cdate = datetime.datetime.now()
    cdateStr = int(str(cdate.year)+str(cdate.timetuple().tm_yday))
    ctime = int(str(cdate.hour)+str(cdate.minute)+str(cdate.second))
    tflag = np.empty([dates.shape[0],data.shape[1],2],dtype='i4')
